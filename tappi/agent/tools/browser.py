...
TOOL_SCHEMA = {
    ...
    "function": {
        ...
        "description": (
            ...
            "Workflow: open a URL → elements (see what's clickable) → click/type/paste → "
            "text (read result).\n\n"
            "## Smart actions (handle low-level details for you)\n"
            "These actions auto-manage focus, verification, and fallbacks internally:\n"
            "- paste: Auto-focuses, inserts, verifies content landed, JS fallback if needed. "
            "PREFERRED for long content (emails, comments, posts). Use 'path' for file content.\n"
            "- type: Auto-focuses (quietly, avoids popups), types, auto-verifies char count. "
            "Good for short fields (search boxes, usernames, subjects).\n"
            "- click: Clicks and reports what changed (navigation, checkbox state, dialog opened).\n\n"
            "## Low-level actions (use when smart actions aren't enough)\n"
            "These are building blocks — you rarely need them directly:\n"
            "- focus: Manually reclaim focus on an element without click events. "
            "Use when type/paste report focus loss and you need to retry.\n"
            "- check: Manually read an element's value + focus state. "
            "Use when you need to inspect state without modifying it.\n"
            "- eval: Run custom JavaScript. Last resort when other actions can't solve the problem.\n"
            "- keys: Raw keyboard events for canvas apps (Sheets, Docs, Figma) where DOM targeting doesn't work.\n\n"
            "## Recovery pattern\n"
            "If a smart action reports a problem (⚠ in response), recover with: "
            "focus(index) → retry the action. If that fails: keys --escape to dismiss overlays → "
            "focus → retry. If that fails: eval for custom JS fix."
        ),
        "parameters": {
            "type": "object",
            "properties": {
                "action": {
                    ...
                    "description": (
                        ...
                        "- elements: List interactive elements (optional: 'selector', 'grep' to filter)\n"
                        ...
                        "- text: Extract visible text (optional: 'selector', 'grep' to filter). "
